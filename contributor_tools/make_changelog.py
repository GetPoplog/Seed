...
parser.add_argument("--type", choices=['debian', 'markdown'], default='markdown')
parser.add_argument("--latest", action="store_true", help="[markdown only] Only output the latest changelog entry")

env = Environment(
    loader=PackageLoader("make_changelog"),
...
def load_changelog_yaml(changelog_yml_path: Path) -> Dict[Any, Any]:
    ...
    for version, body in changelog.items():
        ...
        if not isinstance(body['datetime'], datetime):
            raise TypeError(f"Expected datetime entry for {version} to be a datetime type, but was {body['datetime'].__class__.__name__}")
...
def main(args):
    ...
    elif args.type == "debian":
        out = generate_debian_changelog(changelog_entries)
    else:
        raise ValueError(f"Unknown changelog type '{args.type}'")
